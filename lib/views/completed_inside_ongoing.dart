...
class CompletedInsideOngoing extends StatefulWidget {
  const CompletedInsideOngoing({Key? key,}) : super(key: key);
                       ...
                       child:
                        ...
                        onExpansionChanged:
                             ...
                             SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
                              ...
                              var clientID=sharedPreferences.getString('client_id');
                                try {
                                var getOnGoingInsideOnGoingListUrl=Uri.parse("${Config.getSubSubLocationProgressApi}$clientID/${projectID??"1"}/${locationIDController.text}/${subLocationID[index]}/ONG");
                                  var res=await http.get(
                                      getOnGoingInsideOnGoingListUrl,
                                      headers:{
                                        "Accept": "application/json",
                                        "Authorization": "Bearer $token",
                                      },
